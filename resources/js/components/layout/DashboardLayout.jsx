...
import {
    ...
    Bell, User, FolderOpen, FileCode, LayoutList,
    Calendar, Clock, AlertCircle, Lock, Search, ChevronLeft, ChevronRight, ChevronDown,
    LogOutIcon, Layers, ListTodo, ClipboardList, Wallet, Activity, CalendarCheck, Key, ShieldAlert
} from 'lucide-react'
...
const settingsMenuItems = [
    ...
    { id: 'expense-categories', label: 'Gider Kategorileri', icon: FolderOpen },
    { id: 'appointment-titles', label: 'Randevu Başlıkları', icon: CalendarCheck },
    { id: 'service-tracking', label: 'Hizmet Takip Kategorileri', icon: Activity },
...
export default function DashboardLayout() {
    ...
    // Auto-expand/collapse settings dropdown based on current page
    useEffect(() => {
        const isSettingsPage = location.pathname === '/settings' || location.pathname.startsWith('/settings?')
        if (isSettingsPage) {
            // Settings sayfasına gidildiyse
            if (!expandedSettings) {
                setExpandedSettings(true) // Dropdown aç
            }
            if (!expanded) {
                setExpanded(true) // Sidebar'ı aç
                localStorage.setItem('sidebar-expanded', 'true')
            }
        } else if (expandedSettings) {
            setExpandedSettings(false) // Settings dışında dropdown'ı kapat
        }
    ...
    const toggleSettings = () => {
        // Eğer sidebar collapsed'sa, otomatik aç
        if (!expanded) {
            setExpanded(true)
            localStorage.setItem('sidebar-expanded', 'true')
        }
        // Dropdown'ı toggle et
        setExpandedSettings(prev => {
            localStorage.setItem('settings-menu-expanded', String(!prev))
            return !prev
        })
    }
    ...
    const buildSidebarContent = (isMobile = false) => (
        <div className="flex flex-col h-full">
            {/* Logo + toggle */}
            <div className={`h-[70px] flex items-center shrink-0 border-b border-[#E5E9F0] dark:border-white/5 ${expanded ? 'px-5 justify-between' : 'justify-center px-0'}`}>
                <div className={`flex items-center gap-3 min-w-0 ${expanded ? '' : 'justify-center w-full'}`}>
                    ...
                    {expanded && (
                        <span className="text-base font-black text-[#1A1A2E] dark:text-white tracking-widest uppercase truncate">Vistore</span>
                    )}
                </div>
                {expanded && !isMobile && (
                    <button
            ...
            {/* Nav items */}
            <nav className={`flex-1 flex flex-col gap-0.5 py-3 px-3 ${expanded || isMobile ? 'overflow-y-auto' : 'overflow-visible'}`}>
                {navItems
                    ...
                    .map((item) => {
                        ...
                        return (
                            <NavLink
                                ...
                                title=""
                            >
                                <IconComp size={18} strokeWidth={isActive ? 2.5 : 1.8} className="shrink-0" />
                                {expanded && (
                                    <span className={`text-sm font-semibold truncate ${isActive ? 'text-white dark:text-[#1A1A2E]' : 'text-[#1A1A2E] dark:text-white'}`}>
                                        {item.label}
                                    </span>
                                )}
                                {isDisabled && (
                                    <span className={`${expanded ? 'ml-auto' : 'absolute -top-0.5 -right-0.5'} w-3.5 h-3.5 rounded-full bg-[#E5E9F0] dark:bg-[#222] flex items-center justify-center shrink-0`}>
                                        <Lock size={8} className="text-[#9097A6]" />
                                    </span>
                                )}
                ...
                {hasPermission('settings.view') && (
                    <div className="flex flex-col gap-0.5">
                        <button
                            ...
                            title=""
                        >
                            <Settings size={18} strokeWidth={location.pathname.startsWith('/settings') ? 2.5 : 1.8} className="shrink-0" />
                            {expanded && (
                                <div className="flex-1 flex items-center justify-between">
                                    <span className={`text-sm font-semibold ${location.pathname.startsWith('/settings') ? 'text-white dark:text-[#1A1A2E]' : 'text-[#1A1A2E] dark:text-white'}`}>
                                        Ayarlar
                                    </span>
                                    <ChevronDown size={16} className={`ml-auto transition-transform ${expandedSettings ? 'rotate-180' : ''}`} />
                                </div>
                            )}
                            {!expanded && <span className="nav-tooltip">Ayarlar</span>}
                        </button>

                        {/* Dropdown Items (Sidebar Expanded) */}
                        {expanded && expandedSettings && (
                            <div className="flex flex-col gap-0.5 pl-2 mt-0.5 border-l-2 border-[#E5E9F0] dark:border-white/5 ml-1.5">
                                {settingsMenuItems.map((item) => {
                                    const IconComp = item.icon
                                    const isActive = location.search === `?tab=${item.id}`
                                    return (
                                        <NavLink
                                            key={item.id}
                                            ...
                                            onClick={() => isMobile && setMobileOpen(false)}
                                            className={`
                                                flex items-center gap-2.5 transition-all duration-200 px-3 py-2 rounded-lg text-xs font-semibold
                                                ${isActive
            ...
            {/* Bottom section */}
            <div className={`flex flex-col gap-0.5 pb-4 px-3 border-t border-[#E5E9F0] dark:border-white/5 pt-3 ${expanded ? '' : 'items-center'}`}>
                {/* Storage indicator */}
                {user?.tenant?.storage_limit > 0 && (() => {
                    const pct = Math.min(100, (user.tenant.storage_used / 1024 / 1024 / user.tenant.storage_limit) * 100)
                    const color = pct > 90 ? '#ef4444' : pct > 70 ? '#f59e0b' : '#905EFC'
                    const usedMb = Math.round(user.tenant.storage_used / 1024 / 1024)
                    return expanded ? (
                        <div className="px-3 py-2.5 mb-1">
                            <div className="flex justify-between text-[10px] font-bold text-[#9097A6] mb-1.5 uppercase tracking-wider">
                                <span>Depolama</span>
                                <span>{usedMb} / {user.tenant.storage_limit} MB</span>
                            </div>
                        ...
                        <div className="relative group w-11 h-11 flex items-center justify-center cursor-default mb-1">
                            <svg width="44" height="44" viewBox="0 0 44 44" className="-rotate-90">
                                <circle cx="22" cy="22" r="18" fill="none" stroke="#E5E9F0" strokeWidth="3" />
                                <circle cx="22" cy="22" r="18" fill="none" stroke={color} strokeWidth="3"
                                    strokeDasharray={`${2 * Math.PI * 18}`}
                                    strokeDashoffset={`${2 * Math.PI * 18 * (1 - pct / 100)}`}
                                    strokeLinecap="round" />
                            </svg>
                            <span className="absolute text-[8px] font-black text-[#9097A6]">{Math.round(pct)}%</span>
                            <span className="nav-tooltip">{usedMb} / {user.tenant.storage_limit} MB</span>
                        </div>
                    )
                })()}
                ...
                >
                    {theme === 'dark' ? <Sun size={18} strokeWidth={1.8} className="shrink-0" /> : <Moon size={18} strokeWidth={1.8} className="shrink-0" />}
                    {expanded && <span className="text-sm font-semibold text-[#1A1A2E] dark:text-white">{theme === 'dark' ? 'Açık Tema' : 'Koyu Tema'}</span>}
                    {!expanded && <span className="nav-tooltip">{theme === 'dark' ? 'Açık Tema' : 'Koyu Tema'}</span>}
                </button>
                ...
                >
                    ...
                    {expanded && <span className={`text-sm font-semibold ${location.pathname === '/profile' ? 'text-white dark:text-[#1A1A2E]' : 'text-[#1A1A2E] dark:text-white'}`}>Profilim</span>}
                ...
                <button
                    ...
                    className={`relative group flex items-center gap-3 rounded-xl transition-all text-[#9097A6] hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500
                        ${expanded ? 'px-3 py-2.5 w-full' : 'w-11 h-11 mx-auto justify-center'}`}
                >
                    <div className={`w-7 h-7 rounded-full bg-[#E5E9F0] dark:bg-white/10 flex items-center justify-center text-xs font-black text-[#1A1A2E] dark:text-white shrink-0 group-hover:bg-red-100 group-hover:text-red-500 transition-all`}>
                        <LogOutIcon size={14} />
                    </div>
                    {expanded && (
                        <div className="flex-1 text-left min-w-0">
                            <div className="text-sm font-semibold text-[#1A1A2E] dark:text-white truncate group-hover:text-red-500 transition-colors">{user?.name}</div>
                            <div className="text-[10px] text-[#9097A6] group-hover:text-red-400 transition-colors">Çıkış Yap</div>
                        </div>
                    )}
                    {!expanded && <span className="nav-tooltip">{user?.name} — Çıkış Yap</span>}
                </button>
                ...
                <div className="flex flex-col h-full overflow-hidden">
                    <div className="h-[70px] flex items-center shrink-0 border-b border-[#E5E9F0] dark:border-white/5 px-5">
                        <div className="flex items-center gap-3">
                            <div className="w-9 h-9 shrink-0 rounded-full bg-gradient-to-br from-[#905EFC] to-[#6B3FD4] flex items-center justify-center text-white font-black text-base shadow-lg shadow-[#905EFC]/30 select-none">V</div>
                            <span className="text-base font-black text-[#1A1A2E] dark:text-white tracking-widest uppercase">Vistore</span>
                        </div>
                    </div>
                    <nav className="flex-1 flex flex-col gap-0.5 py-3 px-3 overflow-y-auto">
                        {navItems.filter(i => hasPermission(i.permission)).map((item) => {
                            const IconComp = item.icon
                            const isActive = location.pathname === item.to || (item.to !== '/dashboard' && location.pathname.startsWith(item.to))
                            return (
                                <NavLink key={item.to} to={item.to} onClick={() => setMobileOpen(false)}
                                    ...
                                    <IconComp size={18} strokeWidth={isActive ? 2.5 : 1.8} className="shrink-0" />
                                    <span className={`text-sm font-semibold ${isActive ? 'text-white dark:text-[#1A1A2E]' : 'text-[#1A1A2E] dark:text-white'}`}>{item.label}</span>
                                </NavLink>
                            )
                        })}
                    </nav>
                    <div className="flex flex-col gap-0.5 pb-4 px-3 border-t border-[#E5E9F0] dark:border-white/5 pt-3">
                        <button onClick={toggleTheme} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[#9097A6] hover:bg-[#E5E9F0] dark:hover:bg-white/10 hover:text-[#1A1A2E] dark:hover:text-white transition-all">
                            {theme === 'dark' ? <Sun size={18} strokeWidth={1.8} /> : <Moon size={18} strokeWidth={1.8} />}
                            <span className="text-sm font-semibold text-[#1A1A2E] dark:text-white">{theme === 'dark' ? 'Açık Tema' : 'Koyu Tema'}</span>
                        </button>
                        <NavLink to="/profile" onClick={() => setMobileOpen(false)}
                            ...
                            <span className="text-sm font-semibold text-[#1A1A2E] dark:text-white">Profilim</span>
                        </NavLink>
                        <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[#9097A6] hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-500 transition-all">
                            <div className="w-7 h-7 rounded-full bg-[#E5E9F0] dark:bg-white/10 flex items-center justify-center text-xs font-black text-[#1A1A2E] dark:text-white">{user?.name?.charAt(0)?.toUpperCase()}</div>
                            <div className="flex-1 text-left">
                                <div className="text-sm font-semibold text-[#1A1A2E] dark:text-white">{user?.name}</div>
                                <div className="text-[10px] text-[#9097A6]">Çıkış Yap</div>
                            </div>
                        </button>
                    </div>
                </div>
                ...
                <header className="h-[70px] shrink-0 flex items-center gap-4 px-5 lg:px-7 bg-white dark:bg-[#111111] border-b border-[#E5E9F0] dark:border-white/5 sticky top-0 z-30">
                    ...
                    <div className="flex-1 max-w-sm hidden md:block">
                        <div className="relative">
                            <Search size={15} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#9097A6] pointer-events-none" />
                            <input
                                type="text"
                    ...
                    <div className="flex items-center gap-2.5 pl-3 border-l border-[#E5E9F0] dark:border-white/10">
                        <div className="text-right hidden sm:block">
                            <div className="text-sm font-bold text-[#1A1A2E] dark:text-white leading-tight">{user?.name}</div>
                ...
                {user?.tenant && user.tenant.id && !user.tenant.is_subscribed && !user.tenant.on_trial && !user.tenant.is_gifted && !user.tenant.is_free && (
                    <div className="bg-amber-50 dark:bg-amber-500/10 border-b border-amber-200 dark:border-amber-500/20 px-6 py-3 flex flex-wrap items-center justify-between gap-3 z-20">
                        <div className="flex items-center gap-3 text-amber-700 dark:text-amber-400">
                            ...
                            <span className="text-sm font-medium">Abonelik süreniz dolmuştur. Yeni işlem yapabilmek için abone olmanız gerekmektedir.</span>
                        </div>
                        <button
