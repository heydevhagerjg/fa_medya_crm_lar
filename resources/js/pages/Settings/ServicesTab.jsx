        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {services.map(s => (
                        <div key={s.id} className="group theme-surface border theme-divider rounded-xl p-5 transition-all hover:border-[#905EFC]/30 hover:shadow-md relative">
                            <div className="flex items-start justify-between">
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[11px] px-2 py-0.5 bg-[#E5E9F0] dark:bg-white/10 theme-text-secondary rounded-md font-bold uppercase tracking-wider transition-colors group-hover:bg-[#905EFC]/10 dark:group-hover:bg-[#905EFC]/10 group-hover:text-[#905EFC]">
                                ...
                                <div className="flex gap-1">
                                    <button onClick={() => openModal(s)} className="p-2 rounded-lg theme-text-secondary hover:text-[#905EFC] hover:bg-[#905EFC]/10 dark:hover:bg-[#905EFC]/10 transition-colors"><Edit2 size={16} /></button>
                    ...
                    {services.length === 0 && (
                        <div className="col-span-full border-2 border-dashed theme-divider rounded-xl py-12 text-center">
                            <Layers size={40} className="mx-auto theme-text-secondary mb-3" />
                            <p className="theme-text-secondary font-medium italic">Henüz hizmet tanımlanmamış.</p>
                        </div>
                    )}
                </div>
            )}
            ...
            <Modal open={modal.open} onClose={() => setModal({ open: false, service: null })} title={modal.service ? 'Hizmet Düzenle' : 'Hizmet Ekle'} size="lg">
                <form onSubmit={e => { e.preventDefault(); saveMutation.mutate() }} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium theme-text-primary mb-1">Hizmet Adı *</label>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className="text-sm font-medium theme-text-primary">Özel Alanlar</label>
                            ...
                            {form.customFields.map((field, idx) => (
                                <div key={idx} className="flex items-center gap-2">
                                    <FieldLabelCombobox
                                        ...
                                        className="flex-1 px-3 py-1.5 border rounded-lg text-sm theme-input"
                                    />
                                    <select value={field.type} onChange={e => updateField(idx, 'type', e.target.value)} className="px-2 py-1.5 border rounded-lg text-sm theme-input">
                                    ...
                                    <button type="button" onClick={() => removeField(idx)} className="theme-text-secondary hover:text-red-500 transition-colors"><Trash2 size={14} /></button>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={() => setModal({ open: false, service: null })} className="flex-1 px-4 py-2.5 border theme-divider rounded-xl text-sm font-medium theme-text-primary transition-colors">İptal</button>
            ...
            <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Hizmet Sil">
                <div className="space-y-4">
                    <p className="theme-text-secondary"><span className="font-semibold theme-text-primary">{deleteConfirm?.name}</span> hizmetini silmek istediğinize emin misiniz?</p>
                    <div className="flex gap-3">
                        <button onClick={() => setDeleteConfirm(null)} className="flex-1 px-4 py-2.5 border theme-divider rounded-xl text-sm font-medium transition-colors">İptal</button>
