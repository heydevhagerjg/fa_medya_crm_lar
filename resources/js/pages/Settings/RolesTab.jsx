    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {rolesLoading ? (
                    <div className="col-span-full py-12 text-center theme-text-secondary"><Loader2 className="animate-spin mx-auto" size={32} /></div>
                ) : roles.length === 0 ? (
                    <div className="col-span-full py-12 text-center theme-text-secondary theme-surface border border-dashed theme-divider rounded-xl">
                        Henüz rol oluşturulmamış.
                    </div>
                ) : roles.map(r => (
                    <div key={r.id} className="theme-surface border theme-divider rounded-xl p-4 flex flex-col group hover:border-[#905EFC]/50 transition-all">
                        <div className="flex items-center justify-between mb-3">
                            <div className="font-semibold theme-text-primary flex items-center gap-2">
                                <Shield size={16} className="text-[#905EFC]" />
                                {r.name}
                            </div>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => openModal(r)} className="p-1.5 theme-text-secondary hover:text-blue-500 rounded-lg"><Edit2 size={14} /></button>
                                <button onClick={() => setDeleteConfirm(r)} className="p-1.5 theme-text-secondary hover:text-red-500 rounded-lg"><Trash2 size={14} /></button>
                            </div>
                        </div>
                        <div className="flex flex-wrap gap-1">
                            {r.permissions.length === 0 ? (
                                <span className="text-[10px] theme-text-secondary italic">Yetki verilmemiş</span>
                            ) : (
                                r.permissions.slice(0, 5).map(p => (
                                    <span key={p.id} className="text-[9px] px-1.5 py-0.5 bg-[#E5E9F0] dark:bg-white/5 theme-text-secondary rounded capitalize">
                                        {p.name.replace('.', ' ')}
                                    </span>
                                ))
                            )}
                            {r.permissions.length > 5 && (
                                <span className="text-[9px] px-1.5 py-0.5 bg-[#F4F5F7] dark:bg-white/5 theme-text-secondary rounded">
            ...
            <Modal open={modal.open} onClose={() => setModal({ open: false, role: null })} title={modal.role ? 'Rolü Düzenle' : 'Yeni Rol Ekle'} size="lg">
                <form onSubmit={e => { e.preventDefault(); saveMutation.mutate(form) }} className="space-y-5">
                    <div>
                        <label className="block text-xs font-medium theme-text-secondary mb-1.5">Rol Adı</label>
                        <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full px-4 py-2 border rounded-xl text-sm theme-input" placeholder="Örn: Muhasebe, Saha Personeli" />
                    </div>

                    <div className="space-y-4">
                        <label className="block text-xs font-medium theme-text-secondary uppercase tracking-tighter">İşlem Yetkileri (Granüler)</label>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin min-h-[100px]">
                            {permsLoading ? (
                                <div className="col-span-full py-12 flex flex-col items-center justify-center gap-2 theme-text-secondary">
                                    <Loader2 className="animate-spin" size={24} />
                                    <span className="text-xs">Yetkiler yükleniyor...</span>
                                </div>
                            ) : Object.keys(groupedPermissions).length === 0 ? (
                                <div className="col-span-full py-12 text-center theme-text-secondary bg-[#F4F5F7]/50 dark:bg-[#111111]/50 rounded-xl border border-dashed theme-divider">
                                    Yetki bulunamadı. Lütfen sistem yöneticisiyle iletişime geçin.
                                </div>
                            ) : (
                                Object.entries(groupedPermissions).map(([group, perms]) => (
                                    <div key={group} className="border theme-divider rounded-xl p-3 bg-[#F4F5F7]/30 dark:bg-gray-950/20">
                                        <div className="text-[11px] font-bold theme-text-secondary uppercase mb-2 border-b theme-divider pb-1">{group}</div>
                                        <div className="space-y-2">
                                            {perms.map(p => (
                                                <label key={p.id} className="flex items-center gap-3 cursor-pointer group/item">
                                                    ...
                                                    />
                                                    <span className="text-xs theme-text-secondary group-hover/item:text-[#1A1A2E] dark:group-hover/item:text-gray-200 capitalize">
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={() => setModal({ open: false, role: null })} className="flex-1 px-4 py-2.5 border theme-divider rounded-xl text-sm font-medium theme-text-primary transition-colors">İptal</button>
            ...
            <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Rolü Sil">
                <div className="space-y-4">
                    <p className="theme-text-secondary"><span className="font-semibold">{deleteConfirm?.name}</span> rolünü silmek istediğinize emin misiniz?</p>
