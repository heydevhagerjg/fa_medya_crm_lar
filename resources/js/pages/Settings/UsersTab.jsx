            ...
            <div
                ...
                className={
                    isHighlighted
                        ...
                        : "theme-surface border theme-divider rounded-xl p-4 flex items-center justify-between group hover:border-[#905EFC]/50 transition-all shadow-sm"
                }
            >
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="text-sm font-semibold theme-text-primary flex items-center gap-2 truncate">
                            {u.name}
                            {u.role === 'ADMIN' && <ShieldCheck size={14} className="text-[#905EFC]" title="Yönetici" />}
                        </div>
                        <div className="text-xs theme-text-secondary truncate">{u.email}</div>
                        <div className="mt-1 flex flex-wrap gap-1">
                            ...
                            {u.roles?.map(r => (
                                <span key={r.id} className="text-[10px] px-1.5 py-0.5 bg-[#E5E9F0] dark:bg-white/5 theme-text-secondary rounded font-bold uppercase flex items-center gap-1">
                ...
                <div className="flex gap-1 items-center">
                    {u.role !== 'ADMIN' && (
                        <button onClick={() => openModal(u)} className="p-2 theme-text-secondary hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                            <Edit2 size={16} />
                        </button>
                    )}
                    {u.id !== currentUser?.id && u.role !== 'ADMIN' && (
                        <button onClick={() => setDeleteConfirm(u)} className="p-2 theme-text-secondary hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                            <Trash2 size={16} />
                        </button>
                    )}
                </div>
        ...
        <div className="space-y-4">
            ...
            {isLoading ? (
                ...
                <div className="space-y-8">
                    {adminUsers.length > 0 && (
                        <div>
                            <h3 className="text-sm font-bold theme-text-primary mb-3 border-b theme-divider pb-2 flex items-center gap-2">
                        ...
                        return (
                            <div key={role.id}>
                                <h3 className="text-sm font-bold theme-text-primary mb-3 border-b theme-divider pb-2 flex items-center gap-2">
                                    ...
                                    {role.name}
                                    <span className="text-[10px] font-normal theme-text-secondary bg-[#E5E9F0] dark:bg-white/5 px-2 py-0.5 rounded-full">{role.permissions?.length || 0} Yetki</span>
                                </h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <Modal open={modal.open} onClose={() => setModal({ open: false, user: null })} title={modal.user ? 'Kullanıcı Düzenle' : 'Yeni Kullanıcı Ekle'}>
                <form onSubmit={e => { e.preventDefault(); saveMutation.mutate(form) }} className="space-y-4">
                    <div>
                        <label className="block text-xs font-medium theme-text-secondary mb-1.5">Ad Soyad</label>
                        <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="theme-input" placeholder="Ahmet Yılmaz" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium theme-text-secondary mb-1.5">E-posta</label>
                        <input type="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="theme-input" placeholder="ahmet@firma.com" />
                    </div>
                    <div>
                        <label className="block text-xs font-medium theme-text-secondary mb-1.5">{modal.user ? 'Yeni Şifre (Boş bırakılabilir)' : 'Şifre'}</label>
                        <input type="password" required={!modal.user} value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} className="theme-input" placeholder="••••••••" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-medium theme-text-secondary mb-1.5">Temel Yetki</label>
                            <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} className="theme-input">
                                <option value="ADMIN">Firma Yetkilisi</option>
                                <option value="USER">Personel</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs font-medium theme-text-secondary mb-1.5">Özel Rol</label>
                            <select value={form.roleId} onChange={e => setForm({ ...form, roleId: e.target.value })} className="theme-input">
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="button" onClick={() => setModal({ open: false, user: null })} className="flex-1 px-4 py-2.5 border theme-divider rounded-xl text-sm font-medium theme-text-primary transition-colors">İptal</button>
            ...
            <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Kullanıcıyı Sil">
                <div className="space-y-4">
                    <p className="theme-text-secondary">
                        <span className="font-semibold theme-text-primary">{deleteConfirm?.name}</span> isimli kullanıcıyı silmek istediğinize emin misiniz?
                    </p>
